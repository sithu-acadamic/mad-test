...
function errorDisplay(data){
    if( data.status === 422 ) {
        ...
        $('.err-msg').remove();
        $(".remove-class").removeClass("has-error");
        ...
        $.each(err, function (key, val) {
            ...
            if(id_arr.length>1){
                ...
                if($('#' + id_arr[0] +'_'+inc).parent().hasClass('input-group')){
                    $('#' + id_arr[0] +'_'+inc).parent().after('<span class="err-msg text-danger">'+val[0]+'</span>');
                }else{
                    $('#'+ id_arr[0] +'_'+inc).after('<span class="err-msg text-danger">'+val[0]+'</span>');
                }
                $('#'+ id_arr[0] +'_'+inc).closest( 'td' ).addClass('has-error remove-class');
            }
            else{
                $('#'+ key).after('<span class="err-msg text-danger">'+val[0]+'</span>');
                $('#'+ key).closest( 'div' ).addClass('has-error remove-class');
            }
        });
    }
}
    ...
    $.ajax({
        ...
        success: function (message) {
            if (typeof(message.success) !== 'undefined') {
                $('#mod_loading').hide();
                //$('#modal_msg_sm').html(message['success']);
                //$('#action_refresh_sm').show();
                callBackSuccess(message);
            }
        },
        error: function (errors) {
            if (typeof(errors.responseJSON.error) !== 'undefined'){
                $('#mod_loading').hide();
                //$('#err_action_refresh_sm').show();
                //$('#modal_msg_sm').html(errors.responseJSON.error);
